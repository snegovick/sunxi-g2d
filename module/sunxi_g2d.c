...
{
	struct sunxi_g2d_ctx *ctx = container_of(ctrl->handler, struct sunxi_g2d_ctx, ctrl_handler);
	struct sunxi_g2d *g2d = ctx->g2d;
	G2D_DEBUG_MSG(g2d, "g2d s ctrl id %i\n", ctrl->id);

	switch (ctrl->id) {
	case V4L2_CID_SUNXI_G2D_OP_SELECT:
		G2D_DEBUG_MSG(g2d, "V4L2_CID_SUNXI_G2D_OP_SELECT: %i\n", ctrl->val);
...
static int g2d_try_ctrl(struct v4l2_ctrl *ctrl)
{
	struct sunxi_g2d_ctx *ctx = container_of(ctrl->handler, struct sunxi_g2d_ctx, ctrl_handler);
	struct sunxi_g2d *g2d = ctx->g2d;
	G2D_DEBUG_MSG(g2d, "g2d try ctrl\n");
...
static int g2d_job_ready(void *priv)
{
  ...
  case G2D_BITBLT:
		...
		break;
	default:
		G2D_DEBUG_MSG(g2d, "g2d job ready OP unknown %i\n", ctx->chosen_g2d_op);
		break;
	}
...
static int g2d_s_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	if (ret) {
		G2D_DEBUG_MSG(g2d, "g2d s fmt err %i\n", ret);
		return ret;
	}

	frm = get_frame(ctx, f->type);
	if (IS_ERR(frm)) {
		G2D_DEBUG_MSG(g2d, "g2d s fmt is err\n");
		return PTR_ERR(frm);
	}

	vq = v4l2_m2m_get_vq(ctx->fh.m2m_ctx, f->type);
	if (vb2_is_busy(vq)) {
		G2D_DEBUG_MSG(g2d, "g2d s fmt is busy\n");
		return -EBUSY;
	}
                           ...
                           struct v4l2_selection *sel)
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	struct g2d_frame *frm;
	G2D_DEBUG_MSG(g2d, "g2d g selection\n");

	frm = get_frame(ctx, sel->type);
	if (IS_ERR(frm)) {
		G2D_DEBUG_MSG(g2d, "g2d s selection is err\n");
		return PTR_ERR(frm);
	}
	...
	case V4L2_SEL_TGT_COMPOSE_BOUNDS:
		if (!V4L2_TYPE_IS_CAPTURE(sel->type)) {
			G2D_DEBUG_MSG(g2d, "g2d s selection type is capture\n");
			return -EINVAL;
		}
		break;
	default:
		G2D_DEBUG_MSG(g2d, "g2d s selection einval\n");
		return -EINVAL;
	}
	...
	case V4L2_SEL_TGT_COMPOSE_BOUNDS:
		...
		break;
	default:
		G2D_DEBUG_MSG(g2d, "g2d s selection 2 einval\n");
		return -EINVAL;
	}
                             ...
                             const struct v4l2_selection *sel)
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	struct g2d_frame *frm;
	G2D_DEBUG_MSG(g2d, "g2d try selection\n");
                           ...
                           struct v4l2_selection *sel)
{
	...
	struct g2d_frame *frm;
	int ret;
	G2D_DEBUG_MSG(g2d, "g2d s selection\n");

	ret = g2d_try_selection(file, priv, sel);
	if (ret) {
		G2D_DEBUG_MSG(g2d, "g2d s selection try selection err: %i\n", ret);
		return ret;
	}
                           ...
                           struct device *alloc_devs[])
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	struct g2d_frame *frm;
	G2D_DEBUG_MSG(g2d, "g2d queue setup\n");
...
static int g2d_buf_prepare(struct vb2_buffer *vb)
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	struct g2d_frame *frm;
	G2D_DEBUG_MSG(g2d, "g2d buf prepare\n");
...
static void g2d_buf_queue(struct vb2_buffer *vb)
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	G2D_DEBUG_MSG(g2d, "g2d buf queue\n");
...
static void g2d_queue_cleanup(struct vb2_queue *vq, uint32_t state)
{
	...
	G2D_DEBUG_MSG(g2d, "g2d queue cleanup\n");
...
static int g2d_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	int ret;
	G2D_DEBUG_MSG(g2d, "g2d start streaming\n");
	...
	if (V4L2_TYPE_IS_OUTPUT(vq->type)) {
		...
		G2D_DEBUG_MSG(g2d, "g2d start streaming ret: %i\n", ret);
		return ret;
	}
...
static void g2d_stop_streaming(struct vb2_queue *vq)
{
	struct sunxi_g2d_ctx *ctx = vb2_get_drv_priv(vq);
	struct sunxi_g2d *g2d = ctx->g2d;
	G2D_DEBUG_MSG(g2d, "g2d stop streaming\n");
	if (V4L2_TYPE_IS_OUTPUT(vq->type)) {
		struct sunxi_g2d_ctx *ctx = vb2_get_drv_priv(vq);
                          ...
                          struct vb2_queue *dst_vq)
{
	...
	struct sunxi_g2d *g2d = ctx->g2d;
	int ret;
	G2D_DEBUG_MSG(g2d, "g2d queue init\n");
	...
	if (ret) {
		G2D_DEBUG_MSG(g2d, "g2d queue init ret1: %i\n", ret);
		return ret;
	}
	...
	if (ret) {
		G2D_DEBUG_MSG(g2d, "g2d queue init ret2: %i\n", ret);
		return ret;
	}
...
static int g2d_setup_ctrls(struct sunxi_g2d_ctx *ctx)
{
	...
	int i;
	G2D_DEBUG_MSG(g2d, "g2d setup ctrls\n");
...
static int g2d_open(struct file *file)
{
	...
	int ret;
	G2D_DEBUG_MSG(g2d, "g2d open\n");

	if (mutex_lock_interruptible(&g2d->dev_mutex)) {
		G2D_DEBUG_MSG(g2d, "g2d open -ERESTARTSYS\n");
		return -ERESTARTSYS;
	}
	...
	if (!ctx) {
		...
		G2D_DEBUG_MSG(g2d, "g2d open -ENOMEM\n");
		return -ENOMEM;
	}
...
static DEVICE_ATTR(debug_level, S_IRUGO | S_IWUSR, g2d_show_debug_level, g2d_store_debug_level);
...
static int sunxi_g2d_runtime_resume(struct device *device)
{
	...
	int ret;
	G2D_DEBUG_MSG(g2d, "g2d runtime resume\n");

	G2D_DEBUG_MSG(g2d, "g2d runtime resume reset_control_deassert\n");
   ...
   * functional range.
	 */
	G2D_DEBUG_MSG(g2d, "g2d runtime resume clk_set_rate_exclusive\n");
	...
	G2D_DEBUG_MSG(g2d, "g2d runtime resume clk_prepare_enable bus\n");
	...
	G2D_DEBUG_MSG(g2d, "g2d runtime resume clk_prepare_enable mod\n");
	...
	G2D_DEBUG_MSG(g2d, "g2d runtime resume clk_prepare_enable ram\n");
	...
	G2D_DEBUG_MSG(g2d, "g2d runtime resume g2d_hw_open\n");
	g2d_hw_open(g2d);
...
{
	struct sunxi_g2d *g2d = dev_get_drvdata(device);
	G2D_DEBUG_MSG(g2d, "g2d runtime suspend\n");
