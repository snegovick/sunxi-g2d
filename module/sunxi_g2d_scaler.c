...
static uint32_t g2d_scaler_calc_fir_coef(uint32_t step)
{
	uint32_t pt_coef;
	uint32_t scale_ratio, int_part, frac_part, fir_coef_offt;

	scale_ratio = step >> (VSU_PHASE_FRAC_BITWIDTH - 3);
	int_part = scale_ratio >> 3;
	frac_part = scale_ratio & 0x7;

  uint32_t vsu_coef_offt_zoom1 = VSU_ZOOM0_SIZE;
  uint32_t vsu_coef_offt_zoom2 = vsu_coef_offt_zoom1 + VSU_ZOOM1_SIZE;
  uint32_t vsu_coef_offt_zoom3 = vsu_coef_offt_zoom2 + VSU_ZOOM2_SIZE;
  uint32_t vsu_coef_offt_zoom4 = vsu_coef_offt_zoom3 + VSU_ZOOM3_SIZE;
  uint32_t vsu_coef_offt_zoom5 = vsu_coef_offt_zoom4 + VSU_ZOOM4_SIZE;

  if (int_part == 0) {
    fir_coef_offt = vsu_coef_offt_zoom1;
  } else if (int_part == 1) {
    fir_coef_offt = vsu_coef_offt_zoom1 + frac_part;
  } else if (int_part == 2) {
    fir_coef_offt = vsu_coef_offt_zoom2 + (frac_part >> 1);
  } else if (int_part == 3) {
    fir_coef_offt = vsu_coef_offt_zoom3;
  } else if (int_part == 4) {
    fir_coef_offt = vsu_coef_offt_zoom4;
  } else {
    fir_coef_offt = vsu_coef_offt_zoom5;
  }

	pt_coef = fir_coef_offt * VSU_PHASE_NUM;
	return pt_coef;
}
                          ...
                          uint32_t outw, uint32_t outh, uint8_t alpha)
{
  ...
  uint32_t incw;
  uint32_t inch;
  uint32_t yhcoeff_offset;
  uint32_t yvcoeff_offset;
  uint32_t chcoeff_offset;
  uint32_t tmp_val;

  enum g2d_scaler_pixel_format scaler_format;

	tmp_val = FIELD_PREP(VS_CTRL_EN, 1);
  tmp_val |= FIELD_PREP(VS_CTRL_COEFF_ACCESS_SEL_EN, 1);

  if (format > G2D_FORMAT_IYUV422_Y1U0Y0V0) {
    tmp_val |= FIELD_PREP(VS_CTRL_FILTER_TYPE, 1);
  }
	g2d_write(g2d, VS_CTRL, tmp_val);
  ...
  tmp_val = FIELD_PREP(VS_OUT_SIZE_WIDTH, outw - 1);
  tmp_val |= FIELD_PREP(VS_OUT_SIZE_HEIGHT, outh - 1);
  g2d_write(g2d, VS_OUT_SIZE, tmp_val);
  
	/* p_reg->out_size.bits.out_width = out_w - 1; */
	/* p_reg->out_size.bits.out_height = out_h - 1; */

  tmp_val = FIELD_PREP(VS_GLB_ALPHA_GLB_ALPHA, alpha);
  g2d_write(g2d, VS_GLB_ALPHA, tmp_val);
  
	/* p_reg->glb_alpha.bits.glb_alpha = alpha; */

  tmp_val = FIELD_PREP(VS_XSIZE_Y_WIDTH, inw - 1);
  tmp_val |= FIELD_PREP(VS_XSIZE_Y_HEIGHT, inh - 1);
  g2d_write(g2d, VS_Y_SIZE, tmp_val);
  
	/* p_reg->y_ch_size.bits.y_width = in_w - 1; */
	/* p_reg->y_ch_size.bits.y_height = in_h - 1; */

	temp = inw << VSU_PHASE_FRAC_BITWIDTH;
	if (outw) {
		do_div(temp, outw);
  } else {
		temp = 0;
  ...
  g2d_write(g2d, VS_Y_HSTEP, yhstep << 1);
	...
	temp = inh << VSU_PHASE_FRAC_BITWIDTH;
	if (outh) {
		do_div(temp, outh);
  } else {
		temp = 0;
  ...
  g2d_write(g2d, VS_Y_VSTEP, yvstep << 1);
	...
	yhcoeff_offset = g2d_scaler_calc_fir_coef(yhstep);
  /* yvcoef_offset = g2d_vsu_calc_fir_coef(yvstep); */

	memcpy(g2d->base + VS_Y_HCOEFF0, &lan2coefftab32_full[yhcoeff_offset], VSU_PHASE_NUM * sizeof(unsigned int));
	/* memcpy(&p_reg->vs_y_ch_hor_filter_coef[0], &lan2coefftab32_full[yhcoef_offset], VSU_PHASE_NUM * sizeof(unsigned int)); */

	switch (format) {
	case G2D_FORMAT_Y8:
		scaler_format = VSU_FORMAT_RGB;
		incw = (inw + 1) >> 1;
		inch = inh;

    tmp_val = FIELD_PREP(VS_XSIZE_Y_WIDTH, 0);
    tmp_val |= FIELD_PREP(VS_XSIZE_Y_HEIGHT, 0);
    g2d_write(g2d, VS_C_SIZE, tmp_val);
    g2d_write(g2d, VS_C_HSTEP, 0);
    g2d_write(g2d, VS_C_VSTEP, 0);
    ...
    memcpy(g2d->base + VS_Y_VCOEFF0, &linearcoefftab32[0], VSU_PHASE_NUM * sizeof(unsigned int));
    ...
    tmp_val = FIELD_PREP(VS_XSTEP_INT, ((inh / outh) < 3) ? 0 : (inh / outh - 3));
    g2d_write(g2d, VS_Y_VPHASE0, tmp_val);
	...
	case G2D_FORMAT_IYUV422_Y1U0Y0V0:{
			incw = (inw + 1) >> 1;
			inch = inh;
			scaler_format = VSU_FORMAT_YUV422;

      tmp_val = FIELD_PREP(VS_XSIZE_Y_WIDTH, incw - 1);
      tmp_val |= FIELD_PREP(VS_XSIZE_Y_HEIGHT, inch - 1);
      g2d_write(g2d, VS_C_SIZE, tmp_val);
      ...
      g2d_write(g2d, VS_C_HSTEP, yhstep);
      g2d_write(g2d, VS_C_VSTEP, yvstep << 1);
			/* p_reg->c_hor_step.dwval = yhstep; */
			/* p_reg->c_ver_step.dwval = yvstep << 1; */

			chcoeff_offset = g2d_scaler_calc_fir_coef(yhstep >> 1);
			/* chcoef_offset = g2d_vsu_calc_fir_coef(yhstep >> 1); */

      memcpy(g2d->base + VS_C_HCOEFF0, &lan2coefftab32_full[chcoeff_offset], VSU_PHASE_NUM * sizeof(unsigned int));
	...
	case G2D_FORMAT_YUV422_PLANAR:{
      ...
      memcpy(g2d->base + VS_Y_VCOEFF0, &lan2coefftab32_full[yvcoeff_offset], VSU_PHASE_NUM * sizeof(unsigned int));
			/* memcpy(&p_reg->vs_y_ch_ver_filter_coef[0], */
	...
	case G2D_FORMAT_YUV420UVC_U1V1U0V0:{
			incw = (inw + 1) >> 1;
			inch = (inh + 1) >> 1;
			scaler_format = VSU_FORMAT_YUV420;
      ...
      g2d_write(g2d, VS_C_HSTEP, yhstep);
      g2d_write(g2d, VS_C_VSTEP, yvstep);
			/* p_reg->c_hor_step.dwval = yhstep; */
	...
	case G2D_FORMAT_YUV411UVC_U1V1U0V0:{
			incw = (inw + 3) >> 2;
			inch = inh;
			scaler_format = VSU_FORMAT_YUV411;
      ...
      chcoeff_offset = g2d_scaler_calc_fir_coef(yhstep >> 2);
    ...
    chcoeff_offset = g2d_scaler_calc_fir_coef(yhstep);
	...
	if (scaler_format == VSU_FORMAT_YUV420) {
		/**
		 * yhphase = 0;
		 * yvphase = 0;
		 * chphase = 0xFFFE0000;
    ...
    g2d_write(g2d, VS_Y_HPHASE, 0);
    g2d_write(g2d, VS_Y_VPHASE0, 0);
    g2d_write(g2d, VS_C_HPHASE, 0xFFFc0000);
    g2d_write(g2d, VS_C_VPHASE0, 0xFFFc0000);
		/* p_reg->y_hor_phase.dwval = 0; */
		/* p_reg->y_ver_phase.dwval = 0; */
		/* p_reg->c_hor_phase.dwval = 0xFFFc0000; */
		/* p_reg->c_ver_phase.dwval = 0xFFFc0000; */
	} else if (format != G2D_FORMAT_Y8) {
    g2d_write(g2d, VS_Y_HPHASE, 0);
    g2d_write(g2d, VS_Y_VPHASE0, 0);
    g2d_write(g2d, VS_C_HPHASE, 0);
    g2d_write(g2d, VS_C_VPHASE0, 0);
